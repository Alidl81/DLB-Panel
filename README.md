...
## پیام خرید داخل کانفیگ خالی

در نسخه DLB، یکی از کانفیگ‌های خالی/اطلاع‌رسانی با متن زیر ساخته می‌شود:

```txt
برای خرید به تلگرام @Ali_dlb404 مراجعه کنید
```

این متن از مقدار `purchaseConfigRemark` داخل فایل `dlb-panel.config.json` کنترل می‌شود.

## دیپلوی روی Cloudflare

راهنمای کامل دیپلوی داخل فایل زیر قرار دارد:
