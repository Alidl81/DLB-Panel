...
## نکته مهم درباره خطای دریافت سورس از گیت‌هاب

در این نسخه، سورس نهایی پنل در زمان Build داخل فایل Deployer جاسازی می‌شود. بنابراین Deployer برای ساخت پنل اصلی دیگر به فایل `dist/zeus.js` روی GitHub وابسته نیست. این تغییر برای Cloudflare Git Deploy ضروری است، چون فایل‌های ساخته‌شده در محیط Build الزاماً به ریپوی GitHub commit نمی‌شوند.
